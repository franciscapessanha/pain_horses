...
for folder in ['frontal', 'tilted','profile']:
	...
	for k in range(N_FOLDS):
		for AUG in ['aug_1.5']:
			sub_path = os.path.join(path,'data_%s' % (AUG))
...
#for pose,ratio in [['frontal', 600/265], ['tilted', 332/600], ['profile', 387/600]]:
#for pose,ratio in [['profile', 600/387]]:
#for AUG in ['aug_3']:
	#for k in range(2,3):
	#['frontal', 600/270], ['tilted', 600/330],
	#for pose,ratio in [['profile', 600/380]]:
		#'frontal', 600/270], ['tilted', 600/330],
for pose,ratio in [['profile', 600/380]]:
	...
	yaw_list = np.load(open(os.path.join(os.getcwd(), 'landmark_detection', '%s_%s_%s_final.pickle' % (pose, AUG, 'yaw')), 'rb'), allow_pickle = True)
	pitch_list = np.load(open(os.path.join(os.getcwd(), 'landmark_detection','%s_%s_%s_final.pickle' % (pose, AUG, 'pitch')), 'rb'), allow_pickle = True)
	roll_list = np.load(open(os.path.join(os.getcwd(), 'landmark_detection', '%s_%s_%s_final.pickle' % (pose, AUG, 'roll')), 'rb'), allow_pickle = True)
...
#%%
	...
	for i in range(len(yaw_list)):
		...
		cv.imwrite(os.path.join(ABS_POSE, pose, 'data_aug_1.5' , str(i) + '.png'), img_resize)
		create_pts(lms_resize, os.path.join(ABS_POSE, pose, 'data_aug_1.5', str(i) + '.pts'))
