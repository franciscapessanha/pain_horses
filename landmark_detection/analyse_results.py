...
def get_results_folds(label, pert,  N_FOLDS = 3):
    ...
    if mode == 'cross_val':
        for k in range(N_FOLDS):
            ...
            with open(os.path.join(os.getcwd(), 'data_aug', 'mean_%s_%s_%s_fold_%d.pickle' % (label, AUG, 'yaw', k)), 'wb') as f:
            ...
            for i, angle in [[1, 'pitch'], [2, 'roll']]:
                ...
                for j, mean in enumerate(all_means[i]):
                    if not np.isnan(mean) and mean != 0:
                        aug_factor = ((mean/all_mean_angles[0])** alpha)
                        if aug_factor > 0:
...
LMS_SYSTEM = 'absolute'
mode = 'cross_val'
for alpha in [0.5, 0.7, 0.9, 1.1, 1.3]:
    AUG = 'aug_%.1f' % alpha
