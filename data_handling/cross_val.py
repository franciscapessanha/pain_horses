...
def get_train_test(angles, label):
	...
	plt.figure()
	rolls_counts, rolls_bins, _ = plt.hist(rolls, bins=36)
	...
	plt.figure()
	pitchs_counts, pitchs_bins, _ = plt.hist(pitchs, bins=36)
	...
	with open(os.path.join(ANGLES, '%s_train_angles.pickle' % label), 'wb') as f:
		# Pickle the 'data' dictionary using the highest protocol available.
		pickle.dump(train_angles, f)

	test_angles = [angles[i] for i in range(len(angles)) if angles[i,0] in test_X]
	with open(os.path.join(ANGLES, '%s_test_angles.pickle' % label), 'wb') as f:
		# Pickle the 'data' dictionary using the highest protocol available.
		...
		for train_index, val_indexes in skf.split(train_X, train_y):
			...
			with open(os.path.join(ANGLES, '%s_val_fold_%d_angles.pickle' % (label, i)), 'wb') as f:
				# Pickle the 'data' dictionary using the highest protocol available.
...
def make_copy(list_imgs, subfolder):
	for img in list_imgs:
		shutil.copy2(os.path.join(DATASET, 'images', img), os.path.join(DATASET, 'cross_val', subfolder, img))
